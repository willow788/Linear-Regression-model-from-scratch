"""
Linear Regression implementation from scratch - Version 2.2
Improvements: 
- Changed loss function to (1/(2*m)) for mathematical correctness
- Changed gradient to (1/m) to match the derivative
- Added loss printing during training
...
class LinearRegression:
    ...
    def fit(self, X, y):
        ...
        for iteration in range(self.iter):
            ...
            # Calculate loss (MSE with 1/(2*m) for mathematical correctness)
            loss = (1/(2*m)) * np.sum(error ** 2)
            self.loss_history.append(loss)
            
            # Gradient of loss with respect to weights
            # Derivative of (1/(2*m)) * sum(error^2) = (1/m) * X. T @ error
